 ...
 if(isset($_GET['uid']))
{
  $uu=2;
    $uid= $_GET['uid'];
 $result = mysqli_query($conn, "UPDATE process SET status=$uu WHERE id=$uid");
 // echo "UPDATE process SET status=$uu WHERE id=$uid";
...
$new=mysqli_query( $conn,$sql );
  ...
  $pid=$rows['ppin'];
$sqls="Select * FROM process Where rpid='$pid'  ORDER BY id DESC LIMIT $start,$limit";
...
{ echo "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";
 ...
 if($row["status"]==1){
            ...
            if ($row["status"]==3) {
              ...
              echo "<td>waithig for Delivery</td>";
           
            }
            else{
              echo "<td>"; echo "<a href=?&uid=".$row["id"].">received</a></td>";
            }
...
$ssql="Select * FROM process Where rpid='$pid'";
